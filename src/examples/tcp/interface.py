...
class TCPClient(asyncore.dispatcher):
    ...
    def handle_read(self):
        data = str(self.recv(512))
        print("Received: "+data)
        self.readbuff.put(data)

    def handle_write(self):
        data = self.writebuff.get()
        print("Sending: "+data)
        self.send(data)
        
    def get(self, timeout = 5000):
        data = self.readbuff.get(timeout = timeout)
        return data
        
    def put(self, item):
        self.writebuff.put(item)
            
    def __len__(self):
        return len(self.readbuff), len(self.writebuff)
